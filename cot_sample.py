...
def gen_point_outputs(args, stats, results_dir):
    ...
    # Unpack stats. 
    point_stats = stats['unconditional_point']
...
def sample_with_cot(model, tokenizer, inputs, max_length=20, temperature=0.7):
    ...
    inputs['attention_mask']  # Get attention mask from inputs
    input_ids.size(0)  # Get the batch size
    ...
    #     # Forward pass through the model to generate logits for the last token in each sequence
    #     outputs = model(input_ids=generated_ids, attention_mask=attention_mask)
    #     logits = outputs.logits[:, -1, :]  # Logits for the last generated token in each sequence
        
    #     # Sample the next token for each sequence in the batch
    #     probs = torch.softmax(logits / temperature, dim=-1)
    #     next_token = torch.multinomial(probs, num_samples=1)  # Sample next token for each sequence
        
    #     # Append the next token to each sequence
    #     generated_ids = torch.cat([generated_ids, next_token], dim=-1)
        
    #     # Update attention mask: extend by 1 for each sequence (new tokens should not be masked)
    #     new_attention_mask = torch.ones(batch_size, 1).cuda()
    #     attention_mask = torch.cat([attention_mask, new_attention_mask], dim=1)

    #     # Stop sampling if all sequences have generated the EOS token
    #     if (next_token == eos_token_id).all():
    #         break

    # # Decode each generated sequence in the batch and return as a list of strings
    # generated_texts = [tokenizer.decode(seq, skip_special_tokens=True) for seq in generated_ids]
    # return generated_texts
...
def random_needle_cot_dataset(args, model, tokenizer, context_len, doc_depth, bias_needle):
    ...
    np.zeros((len(dataset), len(args.choices)))
    datasets = []
