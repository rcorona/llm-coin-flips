...
def structure_chat(args, instruct_model):
    """
    ...
    if instruct_model:
        chat = {'user': args.instruct_prompt, 'assistant': args.input_prompt}
    else:
        chat = {'user': args.input_prompt}
...
def run_random_needle_exp(args, model, tokenizer, context_len, doc_depth, bias_needle, expected_dist, instruct_model):

    # Create dataset.
    dataset = create_random_needle_dataset(args, tokenizer, context_len, doc_depth, bias_needle, instruct_model)
    ...
    stats = {
        ...
        'first_token_agg_log_prob': first_token_agg_log_prob.cpu().numpy()
    }
...
def run_point_experiments(args, model, tokenizer, stats, stats_path, instruct_model, exp_dir):
    """
...
if __name__ == '__main__':
    ...
    run_point_experiments(args, model, tokenizer, stats, stats_path, instruct_model, results_dir)
